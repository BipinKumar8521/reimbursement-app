...
const ReimbursementsScreen = ({ navigation }) => {
	...
	const loadExpenses = async () => {
		try {
			...
			if (storedExpenses) {
				...
				const reimbursementExpenses = allExpenses.filter(
					(expense) => expense.reimbursed
				);
				setExpenses(reimbursementExpenses);
			}
			setLoading(false);
		} catch (error) {
			...
			<ScrollView style={styles.expenseList}>
				{loading ? (
					...
					expenses.map((expense) => (
						<TouchableOpacity key={expense.id} onPress={() => handleExpensePress(expense)}>
							<ExpenseItem {...expense} />
						</TouchableOpacity>
					))
				) : (
					<View style={styles.noExpensesContainer}>
						<Text style={styles.noExpensesText}>No reimbursements found</Text>
					</View>
				)}
			</ScrollView>
			<ExpenseModal
